 ...
 * استخدام: canActivate: [roleGuard('pro')]
 *
 * - لو مش مسجل دخول     → يرجع لـ /login
 * - لو مسجل بـ role غلط  → يرجعه لمكانه الصح (client/pro) بدل ما يدخل مكان مش بتاعه
 */
export function roleGuard(requiredRole: UserRole): CanActivateFn {
  return () => {
    ...
    if (user.role !== requiredRole) {
      ...
      const correctPath = user.role === 'pro' ? '/pro/dashboard' : '/find-services';
